    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu])
    {
        ...
        { // this is a bypass that does not fill the LLC
            ...
            if (warmup_complete[fill_cpu])
            {
                ...
                // if(MSHR.entry[mshr_index].data)
                // cout <<" IP: " <<MSHR.entry[mshr_index].ip << " VA: " << MSHR.entry[mshr_index].full_virtual_address <<" current_core_cycle: " << current_core_cycle[fill_cpu] <<  " cycle_enqueued: " << MSHR.entry[mshr_index].cycle_enqueued << " miss_latency: " << current_miss_latency <<  "\n";
        ...
        if (do_fill)
        {
            ...
            else if ((cache_type == IS_L1D) && (MSHR.entry[mshr_index].type != PREFETCH))
            {
                ...
                {
                    PROCESSED.add_queue(&MSHR.entry[mshr_index]);
                    int rob_index = MSHR.entry[mshr_index].rob_index;
                    auto entry = ooo_cpu[cpu].ROB.entry[rob_index];
                    ooo_cpu[cpu].ROB.entry[rob_index].processed_cycle = current_core_cycle[cpu];
                    ooo_cpu[cpu].ROB.entry[rob_index].processed_by = "L1D";
                }
#endif
            }
        ...
        if ((tlb.PROCESSED.entry[tlb.PROCESSED.head].event_cycle <= current_core_cycle[cpu]))
        {
            ...
            if (tlb.PROCESSED.entry[index].l1_rq_index != -1)
            {
                ...
                if ((tlb.PROCESSED.entry[index].full_addr) >> LOG2_PAGE_SIZE == (RQ.entry[rq_index].full_addr) >> LOG2_PAGE_SIZE)
                {
                    ...
                    ooo_cpu[cpu].ROB.entry[RQ.entry[rq_index].rob_index].translated_cycle = current_core_cycle[cpu];
            ...
            { // read hit
                ...
                if (cache_type == IS_ITLB)
                {
                    ...
                    if (PROCESSED.occupancy < PROCESSED.SIZE)
                    {
                        ...
                        int rob_index = RQ.entry[index].rob_index;
                        auto entry = ooo_cpu[cpu].ROB.entry[rob_index];
                        ooo_cpu[cpu].ROB.entry[rob_index].processed_by = "ITLB";
                    }
                }
                else if (cache_type == IS_DTLB)
                {
...

                    {
                        ...
                        int rob_index = RQ.entry[index].rob_index;
                        auto entry = ooo_cpu[cpu].ROB.entry[rob_index];
                        ooo_cpu[cpu].ROB.entry[rob_index].processed_by = "DTLB";
                    }
                }
                else if (cache_type == IS_STLB)
                {
                    ...
                    if (temp.l1_pq_index != -1) //@Vishal: Check if the current request is sent from L1D prefetcher //TODO: Add condition to not send instruction translation request
                    {
                        ...
                        {
                            PROCESSED.add_queue(&temp);
                            ooo_cpu[cpu].ROB.entry[RQ.entry[index].rob_index].processed_by = "STLB";
                        }
                        else
                            assert(0);
                    }
                ...
                else if (cache_type == IS_L1I)
                {
                    ...
                    {
                        PROCESSED.add_queue(&RQ.entry[index]);
                        ooo_cpu[cpu].ROB.entry[RQ.entry[index].rob_index].processed_by = "L1I";
                    }
                }
                // else if (cache_type == IS_L1D) {
