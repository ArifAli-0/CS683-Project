...
void O3_CPU::decode_and_dispatch()
{
    ...
    for (uint32_t i = 0; i < DECODE_BUFFER.SIZE; i++)
    {
        ...
        if (((!warmup_complete[cpu]) && (ROB.occupancy < ROB.SIZE)) || ((DECODE_BUFFER.entry[DECODE_BUFFER.head].event_cycle != 0) && (DECODE_BUFFER.entry[DECODE_BUFFER.head].event_cycle < current_core_cycle[cpu]) && (ROB.occupancy < ROB.SIZE)))
        {
            ...
            ROB.entry[rob_index].execute_begin_cycle = current_core_cycle[cpu];
...
int O3_CPU::execute_load(uint32_t rob_index, uint32_t lq_index, uint32_t data_index)
{
    ...
    ROB.entry[rob_index].l1d_issue_cycle = current_core_cycle[cpu];
...
void O3_CPU::complete_data_fetch(PACKET_QUEUE *queue, uint8_t is_it_tlb)
{
    ...
    { // L1D
        ...
        else
        {
            ...
            ROB.entry[rob_index].num_mem_ops--;
            ROB.entry[rob_index].event_cycle = queue->entry[index].event_cycle;
            // cout << ROB.entry[rob_index].ip << " " << ROB.entry[rob_index].virtual_address << " " << ROB.entry[rob_index].physical_address << " " << ROB.entry[rob_index].event_cycle;
...
void O3_CPU::retire_rob()
{
    for (uint32_t n = 0; n < RETIRE_WIDTH; n++)
    {
        ...
        ROB.entry[ROB.head].retired_cycle = current_core_cycle[cpu];
        for (uint32_t i = 0; i < NUM_INSTR_SOURCES; i++)
        {
            if (ROB.entry[ROB.head].source_memory[i]){
                // cout << "IP: " << ROB.entry[ROB.head].ip << " Load_virtual_address: " << ROB.entry[ROB.head].source_virtual_address[i] << " Execute_begin_cycle: " << ROB.entry[ROB.head].execute_begin_cycle << " Execute_end_cycle: " << ROB.entry[ROB.head].execute_end_cycle << " L1D_issue_cycle: " << ROB.entry[ROB.head].l1d_issue_cycle << " Translated_cycle: " << ROB.entry[ROB.head].translated_cycle << " Processed_cycle: " << ROB.entry[ROB.head].processed_cycle << " Processed_by: " << ROB.entry[ROB.head].processed_by  << " Retired_cycle: " << ROB.entry[ROB.head].retired_cycle << "\n";
            }
        }
